...
const reservationController ={
         ...
         */
        /**
         * Get All hotel example
         * localhost:3000/hotel
         * 
         */
        /**
         * Filter Query Example
         * localhost:3000/hotel?departureDate=2018-04-22&hotelName=test4&arrivalDate=2018-04-20
         */
        getAllHotel: function (req, res) {
            /*   GET /hotel?hotelName=X&arrivalDate=Y&departureDate=Z –
               Returns all hotel that match the search criteria */
            let filter = {};
            let obj = req.query;
            obj.name ? (filter['name'] = obj.name.trim()) : '';
            obj.available ? (filter['available'] = obj.available.trim()) : '';
            obj.city ? (filter['city'] = obj.city.trim()) : '';
            obj.cover ? (filter['cover'] = obj.cover.trim()) : '';


            return reservationService.getAllHotel(filter).then(function (data, err) {
                if (!err) {
                    res.send({
                        hotel: data
                    });
